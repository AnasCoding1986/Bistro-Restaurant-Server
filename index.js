...
async function run() {
  try {
    ...
    app.post('/users', async (req, res) => {
      ...
      // you can do this by many WebAssembly(1. email unique, 2. upsert, 3.simple checking )
    ...
    app.patch('/users/admin/:id', verifyToken, verifyAdmin, async (req, res) => {
      ...
      try {
        ...
        if (result.modifiedCount === 1) {
          res.status(200).send({ message: 'User updated successfully', modifiedCount: result.modifiedCount });
        } else {
          res.status(404).send({ message: 'User not found', modifiedCount: result.modifiedCount });
        }
      } catch (error) {
